...
|---|---|
| Top review: "Please provide a diagnostic dump of such a device that I can verify the datatypes and details." | Posted [issue comment #4416017065](https://github.com/matter-js/matterjs-server/pull/630#issuecomment-4416017065) — full `BasicInformation`, `Descriptor` on endpoint 1, all 7 vendor-cluster data attributes with live values + types, plus `AttributeList` confirming the PR's cluster definition matches the device 1:1. Serial redacted; MAC not included. |
